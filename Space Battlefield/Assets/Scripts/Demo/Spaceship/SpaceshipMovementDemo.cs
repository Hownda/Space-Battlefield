...
public class SpaceshipMovementDemo : MonoBehaviour
{
    ...
    private void MovementInput()
    {
        ...
        // UpDown
        if (!GetComponent<ObjectGravityDemo>().isGrounded)
        {
        ...
        else if (GetComponent<ObjectGravityDemo>().isGrounded && upDown1D >= 0)
        {
            GetComponent<ObjectGravityDemo>().enabled = false;
            Quaternion upDownRotation = transform.rotation;
            upDownRotation *= Quaternion.Euler(0, 0, upDown1D * upDownForce * Time.deltaTime);
        ...
        else if (GetComponent<ObjectGravityDemo>().isGrounded && upDown1D < 0 || GetComponent<ObjectGravityDemo>().isGrounded && thrustPercent < 10)
        {
    ...
    {
        Instantiate(playerPrefab, new Vector3(transform.position.x + 3, transform.position.y, transform.position.z), Quaternion.Euler(Vector3.zero));
    }
