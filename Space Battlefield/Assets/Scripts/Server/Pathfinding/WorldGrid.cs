...
public class WorldGrid : MonoBehaviour
{
    ...
    private void AddNeighbour(GridPoint p, Vector3Int neighbour)
    {
    ...
    private void InitializeGrid()
    {
        ...
        Grid = new GridPoint[GridWidth][][];
        for (int i = 0; i < GridWidth; i++)
        {
            Grid[i] = new GridPoint[GridHeight][];
            for (int j = 0; j < GridHeight; j++)
            {
                Grid[i][j] = new GridPoint[GridLength];
    ...
    public List<GridPoint> GetFreePoints()
    {
        List<GridPoint> freePoints = new List<GridPoint>();
